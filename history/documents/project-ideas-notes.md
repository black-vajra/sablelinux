...
Researches jurisdiction-specific legality of OSINT targeting (CFAA, GDPR, local laws)
Determines which OSINT techniques are legally applicable to that specific target type
Executes permitted passive recon tools (theHarvester, whois, DNS, etc.)
Synthesizes a structured report: legal framework + findings + recommended next steps
